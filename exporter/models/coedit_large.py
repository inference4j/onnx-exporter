...
class CoeditLarge(ExportedModel):
    name = "coedit-large"
    repo_id = "inference4j/coedit-large"
    source_repo = "grammarly/coedit-large"
    ...
    def stage(self, staging_dir: Path) -> None:
        ...
        print("  Exporting CoEdIT Large to ONNX (encoder-decoder with KV cache)...")
        main_export(
...
ONNX export of [CoEdIT Large](https://huggingface.co/grammarly/coedit-large) \
(780M parameters) with encoder-decoder architecture and KV cache support.

CoEdIT is a T5-based model fine-tuned for text editing tasks including grammar \
correction, simplification, coherence, and paraphrasing.
...
Original model by [Grammarly](https://huggingface.co/grammarly).
"""


register(CoeditLarge())
