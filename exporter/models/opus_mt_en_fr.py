...
class OpusMtEnFr(ExportedModel):
    name = "opus-mt-en-fr"
    repo_id = "inference4j/opus-mt-en-fr"
    source_repo = "Helsinki-NLP/opus-mt-en-fr"
    ...
    def stage(self, staging_dir: Path) -> None:
        ...
        print("  Exporting MarianMT en→fr to ONNX (encoder-decoder with KV cache)...")
        main_export(
...
# MarianMT English→French — ONNX

ONNX export of [Helsinki-NLP/opus-mt-en-fr](https://huggingface.co/Helsinki-NLP/opus-mt-en-fr) \
with encoder-decoder architecture and KV cache support.

Trained on the OPUS parallel corpus for English to French translation.
...
- **Repository:** [Helsinki-NLP/opus-mt-en-fr](https://huggingface.co/Helsinki-NLP/opus-mt-en-fr)
- **License:** Apache 2.0
...
try (var translator = MarianTranslator.builder()
        .modelId("inference4j/opus-mt-en-fr").build()) {
